		...
		phoneLoginInterface.setPostListener(new requestVerCodeListenner() {
			...
			public void success(String A) {
				...
				if (code_statusmsg == 1) {
					...
					Log.e("PhoneLoginActivity","返回验证码是否发送成功======" + code.getCode());
					//发送一次后再60s内不能再发送第二次
					mPhoneLogin_OK.post(new Runnable() {
						...
						public void run() {
							...
							if (sum > 0) {
								...
								mPhoneLogin_code.addTextChangedListener(new TextWatcher() {
...
//					overridePendingTransition(0, 0);
	...
	private void NiftyDialogBuilder() {
		...
		niftyDialogBuilder.withTitle("提示").withTitleColor("#000000")
				...
				// 设置对话框背景颜色
				.withMessage("登录失败，该手机还未注册!/n是否进行注册？")
				// 对话框提示内容
	...
	private void PhoneLogin_OK() {
		...
		}else
		{
			...
			if (Integer.valueOf(code_1).equals(phoneLogin_codeback)) {
				...
				Toast.makeText(PhoneLoginActivity.this, "验证码错误！",
